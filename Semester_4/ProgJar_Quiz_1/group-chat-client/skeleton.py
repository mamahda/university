...
class ChatClient:
    def __init__(self, nickname=None, host='127.0.0.1', port=65432):
        # provide defaults so tests can instantiate with only a nickname
        self.host = host
        self.port = port

        # create socket (will be a mock in tests when socket.socket is patched)
        self.client_socket = socket.socket()

        # store encoded nickname (allow passing either str or bytes)
        if nickname is None:
            # if no nickname provided, try to read from input for interactive use
